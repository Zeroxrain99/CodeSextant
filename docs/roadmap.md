...
|---|---|---|
| 重造輪子 | `check → rebuilt` compares bodies, so a wheel reinvented *and renamed* is caught | **measured, and the shape of the problem is not what was assumed.** exp13: one added function in six repeats an existing shape, but 0.994 of those are reachable from the name — same name (0.661) or sharing a word (0.333). Comparing bodies uniquely buys the remaining 0.006. `preflight`'s exact-spelling check reaches almost none of the reachable third, which is where the work is |
...
|---|---|---|
| D1 · reuse before the code exists | `preflight`'s reuse check compares spellings, so an equivalent under an unrelated name never had a chance | **ceiling measured — exp13, and it redirects the item.** One added function in six (0.160 pooled) repeats a shape already in the tree, so the failure is common. But 0.661 of those carry the *same* name and another 0.333 share a word; across 1,052 added functions in six repositories exactly **one** shared no word with what it repeated. So the unreachable-from-a-name case that motivates comparing bodies is 0.001 of added functions, while `preflight`'s exact-spelling check found 0 of 18 differently-named duplicates on requests. **The margin is in name matching, not in body comparison** — the largest unexploited gap measured anywhere in `experiments/` |
