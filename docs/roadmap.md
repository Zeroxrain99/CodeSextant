...
`plan.md` wins and this file is the one that is wrong. Nothing gets a row here without
a problem in `plan.md` §2 to point at.
...
|---|---|---|
| 重造輪子 | `check → rebuilt` compares bodies, so a wheel reinvented *and renamed* is caught | **measured, and better than assumed.** exp13: one added function in six repeats an existing shape, 0.994 of those are reachable from the name, and `preflight`'s name check reaches **0.915 held out** at 5.5 candidates named. Comparing bodies uniquely buys the remaining 0.006 — real, and not what makes the section worth having |
...
| C4 · schema, migration and database constraints | `NOT NULL`, `UNIQUE`, `ForeignKey`, `UniqueConstraint` | **built, as a `guards` kind rather than a gate.** The blocker was never the corpus: adding alembic — a migration tool — produced no `.sql` file either, because a Python project writes its constraints in Python. Counted from the AST, alembic holds **1,023 across 28 files, touched in 0.325 of commits**, the same order as every other guard kind there put together. It is a `constraint` kind inside `guards`, reached by the same measured tiers, **and it has no retrieval score**: none of the six repositories exp9 runs on has one, and scoring it on alembic would spend the prevention corpus's independence. A database-bearing repository added to the *retrieval* corpus is what would settle it |
...
|---|---|---|
| D1 · reuse before the code exists | `preflight`'s reuse check compares spellings, so an equivalent under an unrelated name never had a chance | **ceiling measured — exp13, and the item closes.** One added function in six (0.160 pooled) repeats a shape already in the tree, so the failure is common. 0.661 of those carry the *same* name and 0.333 share a word; across 1,052 added functions exactly **one** shared no word with what it repeated. And the shipped matcher already reaches **0.915 of them held out** while naming 5.5 candidates. Loosening it to overlap-over-the-shorter-name reaches 0.983 for **55.8 names per query** — ten times the output for +0.068, far past where exp1 found a section stops being read. Measured and refused. *(A first version of this row claimed the opposite, from exp3's "0 of 18 on requests" — a number about a different population. See the retraction in `experiments/README.md`.)* |
...
| E1c · what the task set can actually show | stratify before spending agents on it | **done — exp15 and exp17, and together they found a bias in the task set E1–E2 rest on.** exp15 stratified the 120 curated tasks: **86% grep-reachable, 12% convention, 2% hidden.** exp17 then asked whether 2% is a fact about software or about the curation, by running exp15's own classifier over **900 real commits and 2,488 companions** — and got **11% hidden on the derivation set and 7% on the prevention set**, the same three repositories the tasks came from. The frozen task set under-represents the only stratum the tool can help with by **three to five times**. `experiments/prevention_tasks.json` is not wrong, but it is not a sample. **The mechanism is not established**: the obvious suspect — exp12's `>= 2 Python files` filter — was checked in exp18 and mostly refused: 82% of hidden tasks touch two or more Python files, so it costs about a fifth of the stratum rather than four fifths. What remains is a two-sigma shortfall against a candidate population that is itself ~7% hidden. Sampling per stratum makes the question moot rather than answering it |
| E1d · rebuild the task set as a sample | select for the stratum instead of around it | **done — exp18, `experiments/prevention_tasks_stratified.json`.** 155 tasks sampled per stratum: **60 grep-reachable, 51 convention, 44 hidden (28%)** against 2 in the frozen set. Same task shape, same repositories, same ground-truth functions imported from exp12 rather than rewritten, so the two sets are comparable. Validated with E1b's three baselines and the same signature: oracle 1.000 everywhere, null 0.000 everywhere, parent 1.000 on the file modes and 0.000 on reuse, no broken tasks. **The frozen set is not replaced** — it is what E1 and E1b measured and it stays. Two strata ran out rather than being padded (flask convention 12 of 20; pytest hidden 6 of 20; alembic hidden 18 of 20), which the report prints. **Known weakness: `rebuilt_the_wheel` is measurable on only 12 of 155 tasks** against 29 of 120 before, because the sampling is on companion reachability and not on reuse. For that one mode the old set is the better instrument, and each set should be read for what it can measure |
| E2 · the A/B | same model, same tasks, tool available against not | **pilot run and thrown away — the checkouts leaked the answer.** Nine pairs were run against checkouts built with `git worktree add --detach <parent>`. A worktree shares the parent repository's object store, so each checkout sat at the parent commit while carrying the entire future: the reference commit was one `git show` away, `git log --all` listed it, `origin/main` was present, and because the instruction handed to the agent **is** the commit message, `git log --all --grep=` reached the answer in a single step. Two further leaks were in the paths — the checkout was named `<repo>_<sha>` inside a home named `<repo>_<sha>_<condition>`, and the prompt prints that path. It surfaced because one agent volunteered that it had read `--stat` on the reference commit; a transcript grep for history probes then found **8 of 19 trials** touching history. **Every number from those nine pairs is withdrawn**, cost included — an agent that has read the diff needs fewer tokens to find the files. Trials kept as `experiments/results/e2_pilot_void_*.json`. The fix is `exp14_prevention_run.checkout_for`: bundle the parent ref out of a full mirror and clone the bundle, so descendants are **absent** — `git cat-file` cannot get object info for the reference commit, `git fsck` reports nothing dangling — while the ancestors co-change needs remain (1404 for alembic, 4050 for flask). `leaks()` asserts it before every trial and `checkout_for` refuses a path naming the sha; both run in the module's self-test |
| E2b · scale the A/B | enough pairs for the interval to mean something | **open, and now the whole of E2.** The nine pilot pairs bought instruments, not evidence: the shim, the runner-reported cost, the stratified set, the paired report and — the one that was missing — `leaks()`. The stratified set holds 155 tasks; the open question is unchanged, how many pairs buy a usable interval on a cost difference of this size and whether `hidden` behaves differently from `grep_reachable`, but there is no pilot direction to carry into it |
