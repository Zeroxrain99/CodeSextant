...
| E2 · the A/B | same model, same tasks, tool available against not | **pilot run — 3 pairs, and the direction is against the tool.** Six trials on the stratified set, both arms, instrumented. The tool was **verified used** in all three `with_tool` arms (4, 5 and 6 invocations, from the shim log, not self-report). Quality identical in every pair: companion recall 0.722 both, guard 0.667 both. Cost higher every time: **+14.4% tokens** pooled (+4.0%, +6.4%, +24.8%), +7.7 tool calls, +122 s. **No significance is claimed and none is available**: with three pairs all of one sign, a paired bootstrap excludes zero by construction. What this establishes is that the instruments work end to end and that the first three pairs point one way. Raw trials in `experiments/results/e2_pilot_trials.json` |
| E2b · scale the A/B | enough pairs for the interval to mean something | **open.** Three pairs is a pilot. The stratified set holds 155 tasks; the question is how many pairs buy a usable interval on a cost difference this size, and whether the `hidden` stratum behaves differently from `grep_reachable` once there are enough of each to tell |
