...
| E2 · the A/B | same model, same tasks, tool available against not | **done — 20 pairs, 40 trials, 3.61M tokens, zero contaminated. No detectable effect, on either quality or cost.** Quality is an identity, not a null: in **20 of 20 pairs both arms edited exactly the same subset of the true companion files** — eleven pairs edited different file sets, but every difference lay outside the truth, so all three modes read 0.000 with a zero-width interval. Tokens pooled **−5.4% (−5,052 [−14,158, +2,711])**, interval includes zero and the sign test is **11 pairs cheaper, 9 dearer**. Per-pair sd is **19,537 tokens, 3.9× the mean it must resolve**; 80% power on 5.4% would need 118 pairs, on 20% just 9 — so **a large effect would have shown and did not**. `hidden`, the stratum the tool exists for, has the point estimate on the **wrong sign** (+3,061). The pilot's "+14.4% against the tool" is withdrawn: it carried a leaked answer *and* a shim that never exported `CODESEXTANT_HOME`, so every `with_tool` arm re-indexed inside its own budget. The tool was genuinely used — **71 invocations, 23 preflight / 25 guards / 23 check, all 20 arms** — and no `without_tool` arm reached the binary. Full write-up and caveats: `experiments/results/E2.md` |
| E2b · scale the A/B | enough pairs for the interval to mean something | **open, and now quantified.** The variance is measured: sd of the per-pair token difference is 19,537 against a mean of −5,052. Resolving the observed effect needs ~118 pairs (~21M tokens at this run's rate), a 10% effect ~35. The prior question is whether it is worth resolving: quality was identical in 20 of 20, so a cost-only win of a few percent is what is on the table. **Decide that before spending, not after** |
