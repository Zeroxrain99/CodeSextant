...
| E1c · what the task set can actually show | stratify before spending agents on it | **done — exp15 and exp17, and together they found a bias in the task set E1–E2 rest on.** exp15 stratified the 120 curated tasks: **86% grep-reachable, 12% convention, 2% hidden.** exp17 then asked whether 2% is a fact about software or about the curation, by running exp15's own classifier over **900 real commits and 2,488 companions** — and got **11% hidden on the derivation set and 7% on the prevention set**, the same three repositories the tasks came from. The frozen task set under-represents the only stratum the tool can help with by **three to five times**. `experiments/prevention_tasks.json` is not wrong, but it is not a sample: E1's selection preferred commits whose companions were clearly linked, which is the easy half |
| E1d · rebuild the task set as a sample | select for the stratum instead of around it | **open, and now worth doing.** exp17 says hidden companions occur on 7–11% of real commits, so a set stratified deliberately can hold a usable number of them rather than two. Sample within strata and report per stratum, so a tie on the grep-reachable majority cannot hide a difference on the minority that matters |
