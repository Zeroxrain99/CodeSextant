...
| E2 · the A/B | same model, same tasks, tool available against not | **ready to run, and blocked only on agents.** The harness now loads the stratified set by default, reports **per stratum** (a real difference on the 44 hidden tasks would be averaged into nothing by the 60 where both arms succeed), and takes tokens as the primary endpoint with success as a guard that the cheaper arm did not get there by doing less. `exp14_prevention_run.py` run directly is a self-test: it builds a worktree, checks both prompts, and feeds `report` trials whose answer is known — a reporting function nobody has run against a known answer can report anything. The `with_tool` prompt is ~135 tokens longer by construction and that debt is left in, so any measured saving is a lower bound |
