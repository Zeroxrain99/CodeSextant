...
| Six shown **by default**, the rest counted not dropped, and `--limit` to see them | exp1's finding that a predictor naming twenty files stops being read — and exp9 priced what that costs: recall at k held out is 0.306 at six against 0.394 at twenty and 0.506 uncapped. Six is not free. It stays the default because whether a longer section is *read* is not something exp9 can test, but a default nobody could raise looked like a ceiling |
