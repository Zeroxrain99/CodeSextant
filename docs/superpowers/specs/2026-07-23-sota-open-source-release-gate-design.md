...
The daemon binds to loopback by default, uses no telemetry by default, and never
uploads source code. Any future telemetry is opt-in and documented at field level.
...
- Dependency licenses are compatible with MIT distribution.
...
- The repository is created under that account with the intended visibility,
  topics, description, license, branch protection, and security settings.
...
The application uses the discretionary ecosystem-impact path. It must not state or
imply that approval is guaranteed; Anthropic retains discretion and capacity limits
under the [Claude for Open Source terms](https://www.anthropic.com/claude-for-oss-terms).
...
7. Define the versioned graph schema and operation single source of truth.
8. Build the Rust kernel vertically: discovery → parse → store → one query →
   incremental update, always checked against the oracle.
9. Adapt Python and TypeScript semantic resolvers behind the adapter contract.
10. Add MCP stdio, then point CLI and HTTP at the same daemon authority.
11. Build the allowlist-based public export and complete packaging, security,
    provenance, privacy scrub, and documentation.
12. Run the pre-registered public competitive benchmark and independent rerun.
13. Dogfood the release candidate, resolve blockers, and obtain user approval.
14. Verify the target GitHub identity, publish, create the release, then prepare the
