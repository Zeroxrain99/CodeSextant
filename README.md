...
**Measured, on 360 real commits.** ([`exp9`](experiments/README.md)) Hold out a file that actually holds a fence, apply the rest of the commit, and ask `guards` what the change is about to meet: it names the held-out file in **0.228 of cases held out** while printing under four fences. Against the version that inherits a file's relevance rather than checking each guard, +0.122 held out; against reading co-change history instead, +0.078; every interval excluding zero.

**And it has a shape of repository it is wrong for.** On jinja it loses to every control, by −0.133 against co-change and −0.250 against simply reading the most-changed files — because a template engine's tests drive their subjects through indirection and never spell the names, so there is nothing for a per-guard rule to match. A third tier reaches fences through co-change for exactly that reason and recovers part of it, ranked below the two that read the fence's own text and printed with the confidence it rests on. `experiments/README.md` carries the full table including the rows where it loses.
