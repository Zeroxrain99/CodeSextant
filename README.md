...
`REBUILT` compares bodies, not names, and before the code is written there is no body to compare — which is why this section can only exist on this side of the edit.

**How much reinvention there is, measured.** ([`exp13`](experiments/README.md)) Replaying real commits and asking whether each newly added function repeats a shape the tree already held: **one in six does** — the failure is common. But 0.661 of those carry the *same* name and another 0.333 share a word with what they repeat (`_fast_exit` against `fast_exit`, `escape_control_codes` against `make_control_codes_readable`). Across 1,052 added functions in six repositories, exactly **one** shared no word at all. So the `seconds_from_clock`-against-`normalise_duration` case that comparing bodies uniquely catches is 0.001 of added functions; what makes this section worth having is the other 99.4%, caught at no extra query. And `preflight`'s name check, asked of that population, already reaches **0.915 of the duplicates held out** while naming 5.5 candidates. Loosening it to catch the rest costs ten times the names for +0.068, so it stays as it is — measured, and refused.
...
Three layers, because hundreds of guards will not fit in any answer: which fences are in reach (six of them, ranked so the one naming what you touched comes first), then the rule for each, then `--full` for the fence's own source — not fetched otherwise. Guards reached by name need **per-guard** evidence: a file that merely mentions your symbol does not drag in the eleven unrelated switches it also contains.
