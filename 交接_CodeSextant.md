...
## -6. ★ SOTA gate — native-kernel Task 5 核心層完成 5a✅ 5b-i✅ 5b-ii✅ 5c✅（HEAD 99b12f6，下一步 Task 6 semantic sidecar；5d 依賴 T6）
...
- **當前 HEAD＝`99b12f6`（native-kernel Task 5c：multiprocess revision publish + lease fencing）＝5a✅ 5b-i✅ 5b-ii✅ 5c✅（Task 5 核心 syntax+MP 層完成，僅 5d 待 Task 6）；worktree 乾淨。**（鏈：…44ea8a3 T5b-i → 7c61269 T5b-ii → 83955e7 dedup → 99b12f6 T5c）
...
- ⚠ native T5c 已知 carry-over（非 blocker，主代理讀碼核實）：① **multiprocess CAS-race harness 用 `wal:false`（rollback-journal）非 production 預設 `wal:true`（WAL）**——非 gap 而是**防禦縱深測試隔離**：writable open `reject_recovery_sidecars`（store.rs:679）在 production WAL 下阻兩 live writer 同開（primary open-time 互斥），test 刻意關此層以單測 lease/CAS fencing（second 層，SQL ACID 故 journal-agnostic 正確性轉移）；production-realistic 多 writer 路徑（dead writer→crash-recovery 清 sidecar→new writer takeover lease）＝lease-takeover tests，journal-independent 已測。② `boot_id`＝`GetTickCount64`-derived 粗略啟發（僅 staleness 訊號非保護，真 PID-reuse guard＝`process_start` FILETIME via `GetProcessTimes`）。③ unix `process_alive_with_start`（/proc）cfg 後 compile 但 Windows host 未跑（graded platform 不觸發）。
...
### next_step — native-kernel Task 6（★ Task 5 核心 syntax+MP 層全鏈完成：5a✅ 5b-i✅ 5b-ii✅ 5c✅；剩 5d 依賴 T6）

**現況**：Task 5 拆 5a-5d（`31f3aff`）+ 5b 再拆 5b-i/5b-ii（`673e7f7`）。**syntax + multiprocess 核心層全完成並主代理親自重跑驗證**（綠證段）：**5a**（`c5f2ad3` schema+migration）→ **5b-i**（`44ea8a3` crash-safe revision engine）→ **5b-ii**（`7c61269` single-process IndexCoordinator，discovery→parse→store 首個 end-to-end 真接線）→ **dedup**（`83955e7`）→ **5c**（`99b12f6` multiprocess CAS/lease/fencing/PID-reuse，含修 2 條 pre-existing concurrency bug）。**Task 5 剩下只有 5d**（semantic-context bundles），而 **5d `Dependencies: Task 6`**（需 resolver 才能真斷言）→ 故 Task 5 可做的到此為止，frontier 移 **Task 6**。

**下一步 = Task 6**（persistent semantic sidecar protocol + 兩 bundled resolver：Jedi〔Python，PyInstaller onedir〕+ ts-morph〔TS/JS〕）：⚠ **大 slice，先讀 plan Task 6 全文**（同 plan 檔 ~行 598-720：Files/Interfaces/Steps）**再由 commander 決定是否拆子任務**（如 Task 5 拆 5a-5d——Task 6 含 sidecar 進程協定 / PyInstaller onedir build / 跨進程 lifecycle + snapshot lease/refcount / Windows handle reap-before-delete / MCP SDK pin，很可能需拆）。Task 6 `Dependencies: Task 5` 現已滿足。從乾淨 `99b12f6` 起、同紀律（red→green／exact-commit 恰列檔／`--locked`／⛔不碰 frozen〔golden/`codesextant/*.py|sql`/oracle tools/所有已綠 native test〕／land-green-or-pristine／⛔不重啟 daemon／不碰 master／⛔不讀競品 source）；派子代理前跑 `pytest tests/release/test_exact_task_commit.py` 綠 + dot-source `tools/exact_task_commit.ps1` + dispatch preamble。⚠ Task 6 含 PyInstaller / node build＝重度 build，考慮 RunPod（本機只 edit/lint/git/輕測）。

**Task 6 後**：**5d**＝semantic-context bundles + invalidation（併 T6 resolver：`semantic_bundles` 表 + `AnalysisContract` semantic 分量 + `IndexPlan/Receipt::resolution_context_digest` 已留 named empty extension point 未 fake）｜**另立 sub-task**：O(changed·depth) Merkle node schema（T5b-ii carry-over ①，動 `snapshot_nodes` 表）｜then Task 7-12（reliability / packaging / lifecycle / query surface）。全綠仍非公開/發布/SOTA 授權。
