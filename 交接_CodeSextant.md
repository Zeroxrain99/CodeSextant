...
- **當前 HEAD＝`c5f2ad3`（native-kernel Task 5a：schema authority + v4→v5→v6 migration engine）＝Task 5a 完成；worktree 乾淨。**（鏈：…9eabd1b T4 impl → 9fb8cb0 T4 plan-change → 31f3aff T5 decomposition note → c5f2ad3 T5a）
- 跨越序（interlock：G0/G1 → quality T1-3 → native T1 → quality T4 → quality T5-9 → **native T2-12** → 回 quality T10-11）：
  ...
  - **native T4**＝`9eabd1b`（bundled 16-language tree-sitter parser registry；per-language Python-oracle parity〔symbols/comments/complexity/fingerprint〕、iterative bounded extraction + QueryCursor limits + PARSER_LIMIT、16 grammar 精確 pin）＋ `9fb8cb0`（reviewed plan change：go re-pin + kotlin carve-out 記錄）← **HEAD**
- 綠證（本 session 2026-07-24/25，⛔ 主代理親自重跑非轉述子代理）：native T2 邊界 `oracle_snapshot.py`/`public_operation_oracle.py --verify` 皆 exit 0；**native T3**（7a2ba99）`discovery_parity` 兩次同 digest `7389bf4a…842`（各 2 passed）+ `discovery_security` 12 passed、commit 10 檔零 oracle-bound；**native T4**（9eabd1b）`cargo test -p codesextant-parser`＝language_registry 5 + parser_limits 14(+1 ignored) + python_oracle 2（含 carve-out guard test）全綠、clippy `-D warnings` exit 0、commit 13 檔零 frozen、`cargo tree -d` 恰 1 個 tree-sitter 0.25.8/go 0.25.0/kotlin-ng 1.1.0/無 fwcd。
...
### next_step — native-kernel Task 5b（5a ✅ done；Task 5 已 plan-owner 核准拆 5a-5d）
...
**推薦拆 5a-5d**：**5a**＝schema authority + v4→v5→v6 migration engine（Task-6-獨立、`codesextant/schema_v5.sql` 凍結、`tests/parity/test_schema_v5_python_rust.py` parity oracle 已綠、最低風險，先做）｜5b＝versioned revision/recovery/single-process index｜5c＝multiprocess CAS/lease/PID-reuse harness｜5d＝semantic-context bundles（Task 6 resolver 後才能真斷言）。**plan-owner 選 split（5a-5d）**。**5a ✅ done `c5f2ad3`**（schema authority + v4→v5→v6 migration engine；主代理重跑 33 store tests 全綠含 schema_authority 9 / migration_parity 1 / migration_rollback 3 / reversed schema_migration 5、clippy 0、10 檔零 frozen；decomposition note `31f3aff`）。**下一步 5b**＝versioned revisions + `ReaderEpoch` snapshot + single-process index/recovery + syntax-bundle `incremental_index` + `crash_recovery`。⚠ **5b 必解 carry-over**：store `python_oracle` test（`rust_store_matches_the_frozen_python_transcript`）red-until-5b＝**pre-existing 非 5a regression**（query surface 仍 stub `BOOTSTRAP_OPERATION_NOT_IMPLEMENTED` + env-gated `CODESEXTANT_TEST_PYTHON`）；5b 需建 store query surface **並決** frozen v5-golden（`python-store-v1.json` 期 `schema_version:5`）↔ native v6 store 的 reconcile（設計判斷：parity 應比 v5 classification surface＝files/symbols/source_class，**非** schema_version literal 或 v6 revision 表）。5c＝multiprocess CAS/lease harness；5d＝semantic bundles（Task 6 後）。從乾淨 `c5f2ad3` 起、同紀律（red→green／exact-commit／lockfile window〔5b 需擴 rusqlite pin〕／不碰 frozen／land-green-or-pristine／不重啟 daemon）。
...
3. 派子代理前把 plan Task 5 全文 + 前置（`pytest tests/release/test_exact_task_commit.py` 綠→dot-source `tools/exact_task_commit.ps1`）+ dispatch preamble 寫進 brief；exact-commit message 依 plan Step 指定。
4. Task 5 綠仍非公開 / 發布 / SOTA 授權。
